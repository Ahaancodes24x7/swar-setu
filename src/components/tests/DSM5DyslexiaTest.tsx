...
export function DSM5DyslexiaTest({
  ...
  const handleAnswer = useCallback((
    ...
    setTimeout(() => {
      console.log(`Answer submitted. CurrentQ: ${currentQuestionIndex}, TotalQ: ${currentSubTest.questions.length}, CurrentSubtest: ${currentSubTestIndex}, TotalSubtests: ${subTests.length}`);
      
      if (currentQuestionIndex < currentSubTest.questions.length - 1) {
        console.log("Moving to next question");
        setCurrentQuestionIndex((prev) => prev + 1);
      } else if (currentSubTestIndex < subTests.length - 1) {
        console.log("Moving to next subtest");
        setCurrentSubTestIndex((prev) => prev + 1);
        setCurrentQuestionIndex(0);
      } else {
        console.log("All subtests completed, starting analysis");
        analyzeResults();
      }
    }, 500);
  ...
  const analyzeResults = useCallback(() => {
    console.log("analyzeResults called");
    ...
    setTimeout(async () => {
      ...
      console.log("Answers count:", allAnswers.length);
      ...
      console.log("Results phase set successfully");
      onComplete?.(analysisResult);

      // Call AI prediction API (non-blocking)
      const finalScore = analysisResult.overallScore;
      const percentage = analysisResult.overallScore;
      const answersArray = analysisResult.answeredQuestions;
      getAIPrediction({
        ...
        toast.error(`Failed to analyze results: ${error instanceof Error ? error.message : 'Unknown error'}`);
        setTestPhase("intro");
      }
    }, 0);
  }, [onComplete]);
